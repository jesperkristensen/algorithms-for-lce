...
\fixme{Introduction: Our Table Lookup Algorithm}
...
\item[\fprintk\textless$t_{k-1}$, ..., $t_1$\textgreater ac$_{\textrm{cache-horizontal-}\textit{op}}$] is a cache optimized variant of our \fprintk\ algorithm, where \textit{op} describes which of the shift or multiplication arithmetic operations are used.
\end{description}
...
\fixme{Results: Add some results on real world files, in a table like Navarro. STATUS: Pizza\&Chili-teksterne er for store. Brug nogle andre?}
...
\pref{fig:plot-fingerprint-cache-vert} shows results of our inter-level cache optimization. We see no improvement on worst case strings, and we see increased query times on average case strings. We conclude that this attempt at cache optimization does not improve practical query times, which could be expected, since it does not improve the asymptotic I/O bund either.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Conclusions on Experimental Results}

\fixme{Results: Conclusions on Experimental Results}
...
Plot results using LCE value as x-axis: Results looks wrong, why? Likely because of queries in a given pattern, v=max(i,j)
...
We could lay out the tree of compression elements in memory as a B-tree\fixme{Compression: hvad hedder dette layout? B-tree??} to optimize the number of I/O's within each phrase, but each node may have more than $B$ children, and in this case it would not improve the number of I/O's.\fixme{Compression: Er det klart?}
