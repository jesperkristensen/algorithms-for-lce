...
In \pref{sec:algorithms} we theorectically describe and analyze the LCE algorithms introduced here in \pref{sec:intro}, and in \pref{sec:results} we analyze results from practical C++ implementations of these algorithms.
...
The ancestors of a node $u$ is $u$ itself as well as any node, which is a parent of an ancestor of $u$. The depth of a node $u$ is one greather than the depth of $u$'s parent, and the depth of the root is zero. A path in a tree begins at node $u$ and ends at node $v$, where $u$ is an ancestor of $v$.
...
The Level Ancestor at level $\ell$ of node $u$, $\LA(\ell,u)$, is the ancestor of $u$, whoes depth is $\ell$. We can preprocess a static tree in $O(n)$ time and space to answer Level Ancestor queries in $O(1)$ time~\cite{level-ancestor}.
...
For a string of length $n$ with aplhabet size $\sigma$, we can preprocess \proc{SuffixNca} in $O(\sortt)$ time by building the suffix tree, preprocessing it for NCA and building the array of label depths. The query time is $O(1)$, since looking up $L_i$ and $L_j$, doing an NCA query, and looking up the label depth in $D$ all takes constant time.
...
\item[\proc{DirectComp}] is the simple \proc{DirectComp} algorithm with no preprocessing and worst case $O(n)$ query time.
\item[\fprintk\textless$t_{k-1}$, ..., $t_1$\textgreater ac] is our new \fprintk\ algorithm using $k$ levels, where $k$ is $2$, $3$ and $\logceil$. The numbers \textless$t_{k-1}$, ..., $t_1$\textgreater\ describe the exact size of fingerprinted substrings at each level.
\item[\fprintk\textless$t_{k-1}$, ..., $t_1$\textgreater bc] is a variant of our new \fprintk\ algorithm, which starts at level $\ell=k-1$ instead of $\ell=0$, as discussed in \pref{sec:improved-avg}.
...
\pref{fig:plot-complookup} shows \proc{DirectCompLookup} with different time/space-tradeoffs together with \proc{LcpRmq}. In average case, \proc{DirectCompLookup} is slower than \proc{DirectComp}, most likely because it needs to do an extra ckeck to find out if it has reached a position for which a result is stored. For short strings, \proc{DirectLookup} is faster than \proc{DirectCompLookup}, since it does not need to check if the value it needs is stored. However, it quickly grows to be as slow as \proc{LcpRmq}, while \proc{DirectCompLookup} does not experience this slowdown. We explain this with that \proc{DirectLookup} reads from its $O(n^2)$ table of results in every query, whereas \proc{DirectCompLookup} only reads from the $O(n)$ string in average case, and thus has a significantly smaller working set.
...
On worst case strings, the \fprintk\ algorithms are signicicantly better than \proc{DirectComp} and somewhat worse than \proc{LcpRmq}. Up until $n=30.000$ the three measured \fprintk\ algorithms have nearly the same query times. Of the \fprintk\ algorithms, the $k=2$ variant has a slight advantage for small strings of length less than around $2.000$. For longer strings the $k=3$ variant performs the best up to strings of length $250.000$, at which point the $k=\logceil$ variant becomes the best. This indicates that for shorter strings, using fewer levels is better, and when the input size increases, the \fprintk\ variants with better asymptotic query times have better worst case times in practice.

On strings with medium aveage LCE values, we see that our \fprintk\ algorithms are faster than both \proc{DirectComp} and \proc{LcpRmq}

We conclude that our new \fprintk\ algorithm acieves a tradeoff between worst case times and average case times, which is better than the existing best \proc{DirectComp} and \proc{LcpRmq} algorithms, yet it is not strictly better than the existing algorithms on all inputs. \fprintk\ is therefore a good choice in cases where both average case and worst case performance is important.
...
In \pref{fig:plot-fingerprint-avglog} we see that on worst case strings, the query time of \fprint[\logceil] degrade a small amount when using the aveage case optimization. Both variations of \fprint[\logceil] show query times, whic are $O(\log n)$ as expected. On average case strings, the query time of \fprint[\logceil] improves significantly when using average case optimization, and both variants show their expected $O(1)$ and $O(\log n)$ average case query times.
...
We conclude that using average case optimization yields a query time improvement on average case strings, which is significantly greater than the query time degration on worst case strings, and that \fprintk\ therefore should use the average case optimization.
...
On average case strings the cache optimization does not change the query times, while on worst case strings and strings with medium size LCE values, cache optimization gives a noticable improvement for large inputs. The cache optimized \fprint[3] variant with shift operations shows an increase in query times for large inputs, which we cannot explain.
...
We can adapt \proc{DirectComp} to work on a compressed string. To work directly on a compressed string, we first need to have ramdom access to the characters of the original string.
...
We can find $\LCE(i,j)$ in $O(|\LCE(i,j)|)$ query time using Level Ancestor. First compare $\textit{label}(\LA(d(u_i)+1,w_i))$ to $\textit{label}(\LA(d(u_j)+1,w_j))$, where $d(u)$ is the depth of node $u$, then compare $\textit{label}(\LA(d(u_i)+2,w_i))$ to $\textit{label}(\LA(d(u_j)+2,w_j))$ and so on until the two characters differ or either $w_i$ or $w_j$ is reached.
...
\begin{theorem}
\tfprintk, where $k$ is a parameter $1\leq k\leq\logceil$, can preprocess a tree of $n$ nodes to answer LCE queries as defined in \pref{sec:tree-def} using $O(k\cdot n^{1/k})$ query time, $O(k\cdot n)$ space, and $O(n\log n)$ preprocessing time.
\end{theorem}
...
\begin{theorem}
\proc{HeavyPathLce} can preprocess a tree of $n$ nodes to answer LCE queries as defined in \pref{sec:tree-def} using $O(\log n)$ query time, $O(n)$ space, and $O(\sortt)$ preprocessing time.
\end{theorem}

\subsubsection{Data Structure}

Let $T$ be a tree with a heavy path decomposition. A path label for a path in $T$ is a concatenation of all edge labels on the path starting at smallest depth and ending at greatest depth. The string $s$ is a concatenation of path labels of all heavy paths in $T$. For a node $u$, $\textit{end}(u)$ points to the node of greatest depth in $u$'s heavy path, $d(u)$ us the depth of $u$, and if $u$ is heavy, $\textit{index}(u)$ is the index in $s$, which stores $\zlabel(u)$. The tree must be preprocessed for constant time NCA and LA, and the string $s$ must be preprocessed for LCE.

\begin{lemma}
The \proc{HeavyPathLce} data structure takes $O(n)$ space.
\end{lemma}
\begin{proof}
The string $s$ contains less than $n$ characters, since the combined length of all heavy paths in a tree of $n$ nodes is less than $n$. The space usage for NCA, LA, LCE, and the extra information about each node (\textit{end}, \textit{index}, \textit{d}, and heavy markings) is all $O(n)$
\end{proof}

\begin{lemma}
We can preprocess \proc{HeavyPathLce} data structure in $O(\sortt)$ time.
\end{lemma}
\begin{proof}
...
When we directly walk the paths as described in \pref{sec:tree-dc}, we start with $v=0$ and increment $v$ by one as long as $\zlabel(\LA(d(u_i)+v+1,w_i))=\zlabel(\LA(d(u_j)+v+1,w_j))$, and we have not yet reached either $w_i$ or $w_j$.

In a \proc{HeavyPathLce} query $\LCE_T(u_i,w_i,u_j,w_j)$ we also start with $v=0$ and do the following repeatedly as long as we have not reached either $w_i$ or $w_j$:
\begin{enumerate}[~~~a.~]
  \item When either $\LA(d(u_i)+v+1,w_i)$ or $\LA(d(u_j)+v+1,w_j)$ is a light node:
  \begin{enumerate}[~1.~]
    \item Increment $v$ by one if $\zlabel(\LA(d(u_i)+v+1,w_i))=\zlabel(\LA(d(u_j)+v+1,w_j))$, otherwise stop and return $v$.
  \end{enumerate}
  \item When both $\LA(d(u_i)+v+1,w_i)$ and $\LA(d(u_j)+v+1,w_j)$ are heavy nodes:
  \begin{enumerate}[~1.~]
    \item Find $v'=\LCE_s(\LA(d(u_i)+v+1,w_i),\LA(d(u_j)+v+1,w_j))$.
    \item Find $m_i = d(\NCA_T(\textit{end}(\LA(d(u_i)+v,w_i)), w_i))-(d(u_i)+v)$.
    \item Find $m_j = d(\NCA_T(\textit{end}(\LA(d(u_j)+v,w_j)), w_j))-(d(u_j)+v)$.
    \item Stop and return $v+v'$, if $v'<\min(m_i,m_j)$.
    \item otherwise increment $v$ by $m$.
  \end{enumerate}
\end{enumerate}

When we resch either $\LA(d(u_i)+v,w_i)=w_i$ or $\LA(d(u_j)+v,w_j)=w_j$, we stop and return $v$.

\begin{lemma}
The \proc{HeavyPathLce} query finds the correct LCE value.
\end{lemma}
\begin{proof}
The query algorithm maintains the invariant $v\leq\LCE_T(u_i,w_i,u_j,w_j)$. Part $a$ simply checks an extra character and increments $v$ if it finds a match. In part $b$, $m_i$ is the number of heavy edges at the beginning of the path from $\LA(d(u_i)+v,w_i)$ to $w_i$, and similar for $m_j$, like illustrated in \pref{fig:tree-heavy-query}. If $v'<\min(m_i,m_j)$ it means that the LCE query found the longest common prefix to be within both of the two paths we are searching, and we have therefore found the LCE value and stop. If $v'\geq\min(m_i,m_j)$, we had a matching prefix which was as long or longer than the part of the heavy path we are interested in, and we therefore need to continue looking outside that heavy path to find the LCE value.
...
\end{lemma}
\begin{proof}
Since there is at most $O(\log n)$ light edges in any simple path in $T$, the two paths we look at contains at most $O(\log n)$ light edges, and part $a$ of the algoritmh is chosen at most $O(\log n)$ times. Part $b$ is never chosen two times without part $a$ being chosen in between, so part $b$ is also chosen at most $O(\log n)$ times. Both part $a$ and $b$ take $O(1)$ time. In step $b.1$ we can use either \proc{SuffixNca} or \proc{LcpRmq} to achieve $O(1)$ time.
\end{proof}
...
\begin{theorem}
\proc{TreeLcpRmq} and \proc{TreeSuffixNca} can preprocess a tree of $n$ nodes to answer LCE queries as defined in \pref{sec:tree-def} using $O(1)$ query time and $O(n^2)$ space.
\end{theorem}
\begin{proof}
\proc{LcpRmq} relies on accessing two suffixes in SA, $i$ and $j$. In \proc{TreeLcpRmq} the two suffixes are pairs $(u_i,w_i)$ and $(u_j,w_j)$, and each of these can be chosen in $O(n^2)$ ways. We can restrict $w_i$ and $w_j$ to be leafs and still be able to answer the tree original LCE query in $O(1)$ time, but with this modification a tree like the one shown in \pref{fig:quadratic-tree} still has $\Omega(n^2)$ ways to choose each pair. The SA and LCP arrays would therefore be of length $O(n^2)$. When we look at \proc{TreeSuffixNca}, the same argument holds for the nessesary number of leafs in the suffix tree.
\end{proof}
...
\proc{LadderLce}, whic we describe in the next section, finds the LCE value of the first half of the paths from $u_i$ to $w_i$ and from $u_j$ to $w_j$ in constant time, similar to how the ladder decomposition is used to jump from the half way node to $v$ in the Level Ancestor algorithm. We have not found a constant time solution to find the LCE value of the second half of these paths, in the samy way jump pointers is used to jump from $u$ node to the half way node node in the Level Ancestor algorithm. We have thus not found a constant time linear space solution for LCE on trees.
...
To make a ladder decomposition of a tree, we first create a ladder of nodes from the root of the tree to the leaf of greatest depth. We then remove the nodes in ladder from the tree and repeat this operation recursively for all remaining subtrees, until all nodes are part of a ladder. Node $u$'s ladder is the ladder which contains $u$ at this point. Finally, we double the length of each ladder by extending it towards the root of the tree. If we reach the root before we have doubled the length of the ladder, we stop at the root. Each node may thus be in multiple ladders.

For any path in a tree of $n$ nodes, we can find $O(\log n)$ ladders, which contain all the nodes in the path. If a node $u$ is half way down a path, $u$'s ladder contains all ancestors of $u$, which are in that path. The combined size of the ladders is at most $2n$.~\cite{level-ancestor}
%% END PRELIMINARY
...
\begin{theorem}
\proc{LadderLce} can preprocess a tree of $n$ nodes to answer LCE queries as defined in \pref{sec:tree-def} using $O(\log n)$ query time, $O(n)$ space, and $O(\sortt)$ preprocessing time.
\end{theorem}

\subsubsection{Data Structure}

The input tree $T$ is preprocessed with a ladder decomposition, and for each ladder, the labels of nodes in the ladder forms a substring of a string $s$. The tree $T$ is preprocessed for constant time LA and NCA queries, and $s$ is preprocessed for constant time LCE queries. For a node $u$, $\textit{index}(u)$ is an index in $s$, where $\zlabel(u)$ is stored for $u$'s ladder.

\fixme{INSERT FIGURE OF DS}
\begin{lemma}
...
The \proc{LadderLce} query is:
\begin{enumerate}
\item Start with $v_i = u_i$ and $v_j = u_j$
\item Find $m_i = d(v_i')-d(v_i)$ and $i = \textit{index}(v_i') - m_i$, where $v_i'$ is the node half way from $v_i$ to $w_i$.
\item Find $m_j$ and $j$ in the same way.
\item Find $v' = LCE_s(i,j)$
\item If $v' < \min(n_i,m_j)$ then stop and return $v'$ plus the distance between $u_i$ and $v_i$. Otherwise set $v_i = v_i'$ and $v_j = v_j'$ and continue from step two.
\item Stop when $v_i = w_i$ or $v_j = w_j$.
\end{enumerate}
...
\end{lemma}
\begin{proof}
The algorithm maintains the invariant that the labels on the path from $u_i$ to $v_i$ is the same as the labels on the path from $u_j$ to $v_j$.

In step two, $i$ refres to a position in $v_i'$'s ladder, and therefore $s[i]$ is the label of $v_i$.
\end{proof}

We split the tree into ladders and do constant time string LCE on each, just like \proc{HeavyPathLce} does. Since we need to look at at most $O(\log n)$ ladders
 For any path in the tree, we need to look at at most $O(\log n)$ different ladders, which gives \proc{LadderLce} a query time of $O(\log n)$.

\fixme{ARGH}

The main ladder of the node at the middle of the path also contains the node at the beginning of the path.

The ladder decomposition has an extra property, which heavy path decomposition does not have: If $\LCE(u_i,w_i,u_j,w_j)$ is less than half the length of the path from $u_i$ to $w_i$ and less than half the length of the path from $u_j$ to $w_j$, the LCE query takes constant time, since it only needs to look at one ladder.
