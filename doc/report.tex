...
We could have left out step one of the query algorithm in \pref{sec:fingerprint-query} and started with $\ell=k-1$, which would change the query pattern as shown in \pref{fig:fingerprint-updown}. This would keep the asymptotic worst case query time of $O(k\cdot n^{1/k})$, while it might improve practical worst case query time, but it would increase our average case query time to $O(k)$.
\ifarticle
