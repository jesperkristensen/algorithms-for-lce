...
The weight of a node $u$ is the size of the subtree rooted at $u$. For a given node $u$, its child of greatest weight is marked heavy, and all other children are marked light. The root is always marked light. An edge going from a node to a heavy child node is heavy, and an edge going from a node to a light child node is light. A path of only heavy edges is a heavy path, and the combined length of all heavy paths in a tree of $n$ nodes is less than $n$. The markings of heavy and light edges and nodes in a tree is a Heavy Path Decomposition. Every simple path in a tree $T$ of size $n$ contains at most $O(\log n)$ light edges. We can construct a Heavy Path Decomposition in WHAT??\fixme{heavy tree time} time.
%% end of preliminary

To answer LCE queries on a tree, we can create a heavy path decomposition\fixme{define heavy path decomposition} of the tree and create a string of the labels of all the heavy paths concatenated. We can then use constant time string LCE on this string. We call this algorithm \proc{HeavyTreeLce}.

\begin{theorem}
\proc{HeavyTreeLce} can preprocess a tree of $n$ nodes to answer LCE queries as defined in \pref{sec:tree-def} using $O(\log n)$ worst case query time, $O(1)$ average case query time, and $O(n)$ space.
\fixme{preproc time}
\end{theorem}
...
\begin{lemma}
The \proc{HeavyTreeLce} data structure takes $O(n)$ space.
\end{lemma}
\begin{proof}
...
\begin{lemma}
We can preprocess \proc{HeavyTreeLce} data structure in $\sortt$ time.
\end{lemma}
\begin{proof}
Preprocessing LCE using \proc{SuffixNca} or \proc{LcpRmq} takes $O(\sortt)$ time. If we walk through the tree and for each leaf append the heavy path ending in that leaf to $s$, we can construct $s$ in $O(n)$ time. The rest of the data structures can be preprocessed in $O(n)$ time.
\end{proof}
...
In a \proc{HeavyTreeLce} query $\LCE_T(u_i,w_i,u_j,w_j)$ we also start with $v=0$ and do the following repeatedly as long as we have not reached either $w_i$ or $w_j$:
\begin{enumerate}[~~~a.~]
...
\begin{lemma}
The \proc{HeavyTreeLce} query finds the correct LCE value.
\end{lemma}
\begin{proof}
...
\begin{figure}[tp]
    ...
    \end{center}
    \caption{\label{fig:tree-heavy-query}How we find $m_i$ and $m_j$ in a \proc{HeavyTreeLce} query.}
\end{figure}
\end{proof}

\begin{lemma}
The \proc{HeavyTreeLce} query takes $O(\log n)$ time.
\end{lemma}
\begin{proof}
