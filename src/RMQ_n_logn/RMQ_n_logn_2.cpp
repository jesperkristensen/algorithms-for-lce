#include "RMQ_n_logn_2.hpp"

const DTidx RMQ_n_logn_2::Catalan[17][17] = {
...
const DT RMQ_n_logn_2::minus_infinity = -2147483648;

DTidx RMQ_n_logn_2::query(DTidx i, DTidx j) {
...
DTidx RMQ_n_logn_2::in_bloc_minimum(const DTidx start, const DTidx end)const{
			DTidx min_idx = start;
 ...
 */
RMQ_n_logn_2::RMQ_n_logn_2(DT* a, DTidx n) {
	this->a = a;
	this->n = n;
	bs = (DTidx) ceil(log2(n) / 2); // calculate block size
